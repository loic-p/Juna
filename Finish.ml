...
let to_string v : T.expr =
  macro "TO_STRING" [ v ]

let from_int v : T.expr =
  macro "FROM_INT" [ v ]

let from_string v : T.expr =
  macro "FROM_STRING" [ v ]
...
let init_block (x : S.variable) (b : S.block) : T.stmt list =
  ...
  | S.CmpOp (v1, op, v2) ->
     ...
     [T.IfElse (res,
                ...
                set_tag x (-3))]
  | S.String s ->
     [T.Expr (T.Assign (evar x, from_string (T.Literal s)))]
...
let rec finish_term (t : S.term) : C.stmt =
  ...
  | S.Print_int (v, t) ->
     T.Compound [
        scall printf [ T.Literal "%d"; to_int (finish_value (S.VVar v)) ];
        finish_term t
      ]
  | S.Print_string (v, t) ->
     T.Compound [
        scall printf [ T.Literal "%s"; to_string (finish_value (S.VVar v)) ];
        finish_term t
      ]
  | S.LetVal (x, v1, t2) ->
     T.Compound [
  ...
  | S.LetBloRec (x, v, b1, t2) ->
     T.Compound (
         T.DeclStmt (declare x (Some (T.InitExpr (alloc b1)))) ::
           T.DeclStmt (declare v (Some (T.InitExpr (evar x)))) ::
           init_block x b1 @
           [ finish_term t2 ]
       )
  | S.Swi (v, bs) ->
     let (bs', def) = finish_branches v bs in
     T.Switch (
         read_tag (S.VVar v),
         bs',
         def
...
and finish_branches v bs =
  (* first check if we have a default *)
  let rec aux acc =
    function
    | [] -> (List.rev acc, None)
    | (S.BrDefault t)::_ ->
       let def = finish_term t in
       (List.rev acc, Some def)
    | (S.BrVar (v', t))::_ ->
       let def =
         T.Compound [
             T.DeclStmt (declare v' (Some (T.InitExpr (evar v)))) ;
             finish_term t
           ]
       in
       (List.rev acc, Some def)
    | b::tl -> aux ((finish_branch v b)::acc) tl
  in
  let (bs', def) = aux [] bs in
...
and finish_branch v b : T.expr * T.stmt =
  match b with
  | S.Branch (tag, xs, t) ->
     (iconst tag,
      T.Compound (read_fields (S.VVar v) xs [finish_term t]))
  | _ -> failwith "Hey this branch should not exist in this function"
...
let finish_program ((decls, main) : S.program) : T.program =
  prototypes (
